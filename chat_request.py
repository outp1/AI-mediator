...
class OpenAIRequest:
    ...
    def send_request(self, prompt, temperature=0, max_tokens=4000, model="text-davinci-003"):
        try:
            ...
            response = requests.post(self.url, data=data, headers=self.headers, proxies=self.proxies)
            ...
            if response.status_code == 200:
                ...
                self.logger.error(f"Failed to get response with status code: {response.status_code}")
                return f"Failed to get response with status code: {response.status_code}"
